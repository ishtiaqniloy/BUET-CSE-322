...
################################################################802.11 in Random topology with random flow
set cbr_size 1000
set cbr_rate 11.0Mb
set cbr_interval [expr (100.0/$packets_per_sec)];# ?????? 1 for 1 packets per second and 0.1 for 10 packets per second

set x_dim 1000
set y_dim 1000
set time_duration 50 ;#50
set start_time 2 ;#100
...
#set nm out/tcp_wireless.nm
set tr out/tcp_wireless.tr
set topo_file out/topo_tcp_wireless.txt
...
#################################################################### NODE MOVEMENTS BELOW

for {set i 0} {$i < [expr $num_row * $num_col ] } {incr i} {
	set dest_x [expr int($x_dim*rand())]
	set dest_y [expr int($y_dim*rand())]
...
proc plotting {tcpsrc plotterfile} {
	...
	$ns_ at [expr $now+0.1] "plotting $tcpsrc $plotterfile"
	
}

for {set i 0} {$i < $num_flows} {incr i} {
	$ns_ at 0.2 "plotting $udp_($i) $congestionFile"
}
