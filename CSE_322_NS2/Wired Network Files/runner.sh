...
do
	...
	node_thr_file="$folder$output_file_format$under$var$under$nodeThr.out"
	rm -f $node_thr_file
	touch $node_thr_file
	...
	do
		if [ $var -eq 1 ]; then
			...
			luther="node variation"
		elif [ $var -eq 2 ]; then 
			flow=$k
			echo "========== Executing $k flow variation =========="
			luther="flow variation"
		elif [ $var -eq 3 ]; then
			npkt=$k
			echo "========== Executing $k packet_rate variation =========="
			luther="packet rate variation"
		else
		...
		flag2=0
			...
			do
			#################START AN ITERATION
			echo "   EXECUTING $(($i+1)) th ITERATION for $k $luther "
...
#incorporate congestion calculation here
	...
	j=0
	echo -1 $variation $node $flow $npkt >> $node_thr_file
	while [ $j -lt $node ]; do
