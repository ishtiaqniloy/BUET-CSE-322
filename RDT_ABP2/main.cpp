...
struct pkt* A_makePacket(struct msg *msg){

    struct pkt *tempPacket = new struct pkt;
    tempPacket->seqnum = A_lastSentSeq^1;
    tempPacket->acknum = -1; //not required in unidirectional transmission
    bufferCopy(tempPacket->payload, msg->data, MSG_SIZE);
...
void A_output(struct msg message){
    ...
    struct pkt *packet = A_makePacket(&message);
...
void A_input(struct pkt packet){
    ...
    else{   ///NACK received. should do nothing ???
...
void A_timerinterrupt(void){
    ...
    printf("***At %f In A_timerInterrupt(): Timeout occured. Retransmitting pkt%d: %s***\n", time, A_lastSentSeq, A_lastPacket->payload);
...
void B_input(struct pkt packet){
    ...
    else if(packet.seqnum == B_lastCorrectSeq){
